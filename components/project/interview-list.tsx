...
import { CalendarDays, Search, Filter, MessageSquare, ArrowRight, ArrowLeft } from "lucide-react"
...
export default function InterviewList({ project, onBack }: InterviewListProps) {
  ...
  const fetchInterviews = async () => {
    try {
      ...
      console.log('인터뷰 데이터 로드 중 - 프로젝트 ID:', project.id);
  ...
  const handleInterviewClick = (index: number) => {
    router.push(`/interviews/${index}?project_id=${project.id}`)
  }
      ...
      >
        <motion.div variants={itemVariants}>
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500 font-medium">프로젝트:</span>
              <span className="text-sm font-medium text-red-500">{project.name}</span>
            </div>
          </div>
        </motion.div>
